...
function transformPlanificacionLB_(shPlanLB, weekMap) {
  ...
  if ([iPersonal, iEtapa, iAct, iTar, iQty, iUni, iRend, iHHTot].some(x => x === -1)) {
    throw new Error('En "PLANIFICACION LB" faltan columnas base. Header detectado en fila ' + (headerRowIdx + 1) + '.');
  }

  const out = [["SEMANAS.WEEK_KEY", "Personal", "ETAPA", "ACTIVIDAD", "TAREA", "Cantidad Teórica", "Unidad", "Rendimiento Teorico", "HH TOTALES", "Avance Real", "Avance HH"]];
  ...
  if (!weekCols.length) {
    throw new Error('En "PLANIFICACION LB" no encontre columnas semanales validas.');
  }
  ...
  for (let r = headerRowIdx + 1; r < v.length; r++) {
    ...
    for (const weekCol of weekCols) {
      ...
      const hhWeek = (hhTot > 0) ? (hhTot * pctWeek) : (rend > 0 ? qtyWeek * rend : 0);
...
function transformAvanceRealMatriz_(sheet, weekMap, headerRowIndex1based, hasCantTotalCol) {
  ...
  const CANT_TOTAL_NAMES = ["Cantidad \nReal", "Cantidad Real", "CANTIDAD REAL", "Cant. Real", "CantReal", "Cantidad Total", "CANTIDAD TOTAL", "Cant. Total", "CantTotal"];

  let iCantTotal = -1;
  if (hasCantTotalCol) {
    for (const name of CANT_TOTAL_NAMES) {
      const idx = h.findIndex(cell => cell.trim().replace(/\s+/g, " ") === name.trim().replace(/\s+/g, " "));
      if (idx !== -1) { iCantTotal = idx; break; }
    }
    if (iCantTotal === -1) {
      Logger.log(`[WARN] "${sheet.getName()}": hasCantTotalCol=true pero no se encontro columna de total conocida.`);
    }
  }
  ...
  if (!weekCols.length) {
    throw new Error(`En "${sheet.getName()}" no encontre columnas semanales validas.`);
  }
...
function writeTable_(sheet, rows) {
  if (!sheet) throw new Error("No existe hoja destino (revisa nombres en Config.js).");
  sheet.clearContents();
...
function weekKeyFromHeader_(header, weekMap) {
  if (header instanceof Date) return weekMap[dateKey_(header)] || null;
  const asDate = new Date(header);
  if (!isNaN(asDate.getTime())) return weekMap[dateKey_(asDate)] || null;
...
function crearTriggerETLNocturno() {
  ...
  ScriptApp.newTrigger('ETL_actualizarTodo').timeBased().atHour(23).everyDays(1).create();
...
function borrarTriggerETLNocturno() {
  ...
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'ETL_actualizarTodo') ScriptApp.deleteTrigger(triggers[i]);
  }
...
function logETL_(data) {
  ...
  sh.appendRow([new Date(), data.status || "", data.fileId || "", data.detail || "", data.planRows || "", data.realRows || ""]);
}

function TEST_driveApiOrigen() {
  const selected = resolveSourceExcelFromFolder_();
  Logger.log("Folder ID: " + SOURCE_EXCEL_FOLDER_ID);
  Logger.log("Archivo seleccionado: " + selected.name + " (" + selected.id + ")");
  Logger.log("mimeType: " + selected.mimeType);
  Logger.log("modifiedDate: " + selected.modifiedDate);
  if (selected.viaShortcutId) Logger.log("Via shortcut: " + selected.viaShortcutId);
}
